...
class CompressLibrary {
    ...
    private function create($image, $name, $type, $size, $c_type, $level) {
        $kompress_path="assets/compressed";

        $im_name = $name;
        $im_output =$kompress_path.'/'.$im_name;
    ...
    function run($image, $c_type, $level = 0) {
        ...
        // cek & ricek
        if(in_array($c_type, array('jpeg','jpg','JPG','JPEG','png','PNG'))) { // jpeg, png, gif only

            if(in_array($im_type, array( 'image/jpeg', 'image/png'))){
                if($level >= 0 && $level <= 9){
